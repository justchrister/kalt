...
ALTER TABLE user_signed_in ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column user_signed_in.message_id 
...
comment on column user_signed_in.message_entity_id 
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'user_signed_in'
      AND policyname = 'SELF — Insert'
  ) THEN
    CREATE POLICY "SELF — Insert" ON public.user_signed_in
