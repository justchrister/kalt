...
comment on column get_user_portfolio__exchange_orders.message_id 
...
comment on column get_user_portfolio__exchange_orders.message_entity_id 
...
comment on column get_user_portfolio__exchange_orders.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column get_user_portfolio__exchange_orders.message_sender 
is 'where the message originates, usually set in the application.';

comment on column get_user_portfolio__exchange_orders.message_read 
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO get_user_portfolio__exchange_orders (message_id, message_entity_id, message_sender, message_created)
