...
CREATE OR REPLACE FUNCTION "sub_paymentCards_acl_stripe"()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION "sub_paymentCards_acl_stripe"();


-- Set up webhook function 

CREATE OR REPLACE FUNCTION "acl/stripe/webhooks/paymentCards"()
RETURNS TRIGGER AS $$
DECLARE 
  response RECORD;
  payload TEXT;
...
AFTER INSERT ON "trigger_paymentCards_acl_stripe"
FOR EACH ROW
