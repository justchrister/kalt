...
CREATE TABLE get_account_balance__user_preferences (
...
ALTER TABLE get_account_balance__user_preferences ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column get_account_balance__user_preferences.message_id 
...
comment on column get_account_balance__user_preferences.message_entity_id 
...
comment on column get_account_balance__user_preferences.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column get_account_balance__user_preferences.message_sender 
is 'where the message originates, usually set in the application.';

comment on column get_account_balance__user_preferences.message_read 
...
CREATE OR REPLACE FUNCTION get_account_balance__user_preferences()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO get_account_balance__user_preferences (message_id, message_entity_id, message_sender, message_created)
...
CREATE TRIGGER get_account_balance__user_preferences
AFTER INSERT ON user_preferences
FOR EACH ROW
EXECUTE FUNCTION get_account_balance__user_preferences();