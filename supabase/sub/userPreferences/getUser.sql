...
CREATE TABLE get_user__user_preferences (
...
ALTER TABLE get_user__user_preferences ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column get_user__user_preferences.message_id 
...
comment on column get_user__user_preferences.message_entity_id 
...
CREATE OR REPLACE FUNCTION get_user__user_preferences()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO get_user__user_preferences (message_id, message_entity_id, message_sender, message_created)
...
CREATE TRIGGER get_user__user_preferences
AFTER INSERT ON user_preferences
FOR EACH ROW
EXECUTE FUNCTION get_user__user_preferences();