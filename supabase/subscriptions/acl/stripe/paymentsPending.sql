...
CREATE TABLE acl_stripe__payments_pending (
...
ALTER TABLE acl_stripe__payments_pending ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column acl_stripe__payments_pending.message_id 
...
comment on column acl_stripe__payments_pending.message_entity_id 
...
comment on column acl_stripe__payments_pending.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column acl_stripe__payments_pending.message_sender 
is 'where the message originates, usually set in the application.';

comment on column acl_stripe__payments_pending.message_read 
is 'the only field that is updated in this table, when the service reads it, it changes the bool to true.';


-- Create the trigger function on the payments_pending
CREATE OR REPLACE FUNCTION acl_stripe__payments_pending()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO acl_stripe__payments_pending (message_id, message_entity_id, message_sender, message_created)
...
CREATE TRIGGER acl_stripe__payments_pending
AFTER INSERT ON payments_pending
FOR EACH ROW
EXECUTE FUNCTION acl_stripe__payments_pending();