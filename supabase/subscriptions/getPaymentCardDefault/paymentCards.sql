...
-- version 6.4.23
-- service get_mayment_card_default
-- topic   payment_cards

--- create the table, with default values
CREATE TABLE get_mayment_card_default__payment_cards (
...
ALTER TABLE get_mayment_card_default__payment_cards ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column get_mayment_card_default__payment_cards.message_id 
...
comment on column get_mayment_card_default__payment_cards.message_entity_id 
...
comment on column get_mayment_card_default__payment_cards.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column get_mayment_card_default__payment_cards.message_sender 
is 'where the message originates, usually set in the application.';

comment on column get_mayment_card_default__payment_cards.message_read 
...
CREATE OR REPLACE FUNCTION get_mayment_card_default__payment_cards()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO get_mayment_card_default__payment_cards (message_id, message_entity_id, message_sender, message_created)
...
CREATE TRIGGER get_mayment_card_default__payment_cards
AFTER INSERT ON payment_cards
FOR EACH ROW
EXECUTE FUNCTION get_mayment_card_default__payment_cards();