...
CREATE TABLE payments__account_transactions (
...
ALTER TABLE payments__account_transactions ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column payments__account_transactions.message_id 
...
comment on column payments__account_transactions.message_entity_id 
...
comment on column payments__account_transactions.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column payments__account_transactions.message_sender 
is 'where the message originates, usually set in the application.';

comment on column payments__account_transactions.message_read 
...
CREATE OR REPLACE FUNCTION payments__account_transactions()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO payments__account_transactions (message_id, message_entity_id, message_sender, message_created)
...
CREATE TRIGGER payments__account_transactions
AFTER INSERT ON account_transactions
FOR EACH ROW
EXECUTE FUNCTION payments__account_transactions();