...
CREATE TABLE pay_revenue__exchange_orders (
...
ALTER TABLE pay_revenue__exchange_orders ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column pay_revenue__exchange_orders.message_id 
...
comment on column pay_revenue__exchange_orders.message_entity_id 
...
comment on column pay_revenue__exchange_orders.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column pay_revenue__exchange_orders.message_sender 
is 'where the message originates, usually set in the application.';

comment on column pay_revenue__exchange_orders.message_read 
...
CREATE OR REPLACE FUNCTION pay_revenue__exchange_orders()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO pay_revenue__exchange_orders (message_id, message_entity_id, message_sender, message_created)
...
CREATE TRIGGER pay_revenue__exchange_orders
AFTER INSERT ON exchange_orders
FOR EACH ROW
EXECUTE FUNCTION pay_revenue__exchange_orders();