...
comment on column pay_revenue__revenue_transactions.message_id 
...
comment on column pay_revenue__revenue_transactions.message_entity_id 
...
comment on column pay_revenue__revenue_transactions.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column pay_revenue__revenue_transactions.message_sender 
is 'where the message originates, usually set in the application.';

comment on column pay_revenue__revenue_transactions.message_read 
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO pay_revenue__revenue_transactions (message_id, message_entity_id, message_sender, message_created)
