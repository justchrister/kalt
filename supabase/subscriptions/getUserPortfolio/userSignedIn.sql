...
CREATE TABLE get_user_portfolio__user_signed_in (
...
ALTER TABLE get_user_portfolio__user_signed_in ENABLE ROW LEVEL SECURITY;

--- Standard descriptions
comment on column get_user_portfolio__user_signed_in.message_id 
...
comment on column get_user_portfolio__user_signed_in.message_entity_id 
...
comment on column get_user_portfolio__user_signed_in.message_created 
is 'when the message was generated, usually set in the application. It can be created in javascript by doing ok.timestamptz()';

comment on column get_user_portfolio__user_signed_in.message_sender 
is 'where the message originates, usually set in the application.';

comment on column get_user_portfolio__user_signed_in.message_read 
is 'the only field that is updated in this table, when the service reads it, it changes the bool to true.';


-- Create the trigger function on the user_signed_in
CREATE OR REPLACE FUNCTION get_user_portfolio__user_signed_in()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO get_user_portfolio__user_signed_in (message_id, message_entity_id, message_sender, message_created)
...
CREATE TRIGGER get_user_portfolio__user_signed_in
AFTER INSERT ON user_signed_in
FOR EACH ROW
EXECUTE FUNCTION get_user_portfolio__user_signed_in();